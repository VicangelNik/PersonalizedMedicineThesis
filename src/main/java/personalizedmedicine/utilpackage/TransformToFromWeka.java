...
import static personalizedmedicine.helpful_classes.Constants.NORMAL_TISSUE;
import static personalizedmedicine.helpful_classes.Constants.PRIMARY_TUMOR;
...
public final class TransformToFromWeka {
    ...
    public static List<ArrayList<Feature>> createEMPCAInputFromWeka(Instances originalDataset) {
        ...
        log.info("Number of distinct features with NaN values: " + distinctElements.size());
        log.info("Number of NaN value occurencies: " + nanFeatureList.size());
        return empcaInput;
    }
    ...
    public static Instances eigensToWeka(DoubleMatrix2D eigenVectors, String nameNewDataset, List<Double> classValues,
        ...
        // add class
        attInfo.add(new Attribute(className, Arrays.asList(PRIMARY_TUMOR, NORMAL_TISSUE)));
    ...
    private static List<String> convertClassDoubleValuesToNominal(List<Double> classValues) {
        ...
        classValues.forEach(x -> {
            if (x.equals(0.0)) {
                attributeValues.add(PRIMARY_TUMOR);
            }
            else {
                attributeValues.add(NORMAL_TISSUE);
            }
        });
