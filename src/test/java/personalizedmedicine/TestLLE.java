    ...
    void TestLLECase() throws IOException {
        ...
        for (int dimensions = 10; dimensions < 201; dimensions += 10) {
            for (int kNearest = 5; kNearest < 500; kNearest += 5) {
                try {
                    val lle = smile.manifold.LLE.of(data, dimensions, kNearest);
                    Assertions.assertTrue(lle.coordinates != null && lle.coordinates.length > 0,
                                          "LLE results should not be null or empty");
                    final Instances reData = TransformToFromWeka.manifoldsToWeka(lle.coordinates, "lleDataset",
                    ...
                    Assertions.assertFalse(lle.graph.getEdges().isEmpty(), "Graph should not be empty");
                    Assertions.assertTrue(lle.index.length > 0, "Original indices should exist.");
                } catch (Exception e) {
